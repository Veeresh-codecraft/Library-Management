...
import { IUserBase, IUser } from "../models/user.models";
...
async function updateUser(repo: UserRepository, UIdToUpdate: number) {
  ...
  if (user?.name != "") user.name = updatedData.name;
  if (user.DOB != "") user.DOB = updatedData.DOB;
  if (user.phoneNum) user.phoneNum = updatedData.phoneNum;
}
